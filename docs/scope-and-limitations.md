...
| Cascade-spend as a discrete service | ✅ Pure `calculateCascadeSpend(input)` function exists (`ppi-wallet-app/src/services/cascade-spend.ts`) with 13 invariant tests | Production: reuse same pure function server-side; wire the returned plan to a real ledger write |
...
2. ~~**No cascade-spend pure function**~~ ✅ **CLOSED.** `calculateCascadeSpend(input) → {splits, status, reason}` shipped in `ppi-wallet-app/src/services/cascade-spend.ts`. Takes immutable input, returns a plan, never writes. 13 tests in `src/services/__tests__/cascade-spend.test.ts` enforce: category-specific priority, Gift fallback rules, expired-Gift exclusion, clean decline with zero partial-debit, no-zero-amount-splits.
